...
export default function useIsFavorited(category: Category, id: number | string | undefined)
    ...
    const onPressFavorite = useCallback(async () => {
        ...
        if (category === Category.Quote) {
            ...
            if (isFavorited)
                dispatch(removeNiceClipFavoritedID(id));
            else
                dispatch(addNiceClipFavoritedID(id));
        }
            ...
            if (isFavorited)
                dispatch(removeAwesomeVideoFavoritedID(id));
            else
                dispatch(addAwesomeVideoFavoritedID(id));
        }
